...
class TopFiveLastMonth extends Component
{
    ...
    {
        $this->draw();
        return view('livewire.dashboard.mechanic.top-five-last-month')
            ->with('topFiveLastMonth',$this->chart);
    }
    ...
    private function draw()
    {
        ...
        $data = RepairLog::query()->selectRaw("users.name,COUNT(repair_log.id) as rCount")
            ...
            ->whereDeleted(0)
            ->whereRaw('MONTH(`repair_log`.`timestamp`) = MONTH(CURRENT_TIMESTAMP) ' . ($month == "Jan" ?  '+ 11' : '- 1'))
            ->whereRaw('YEAR(`repair_log`.`timestamp`) = YEAR(CURRENT_TIMESTAMP) ' . ($month == "Jan" ?  '- 1' : ''))
